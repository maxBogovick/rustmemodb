...
async fn comprehensive_performance_benchmark() {
    ...
    db.execute("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, email TEXT UNIQUE)")
        .await
        .unwrap();
    ...
    assert!(
        duration_insert.as_secs() < 10,
        "Insert performance is too slow! O(N) check might be active."
    );
    ...
    let result = db
        .execute(&format!("SELECT * FROM users WHERE id = {}", target_id))
        .await
        .unwrap();
    ...
    assert!(
        duration_lookup < Duration::from_millis(5),
        "Index lookup too slow"
    );
    ...
    for i in 0..join_row_count {
        db.execute(&format!("INSERT INTO orders VALUES ({}, {}, 99.99)", i, i))
            .await
            .unwrap();
    }
    ...
    println!(
        "✅ Joined {}x{} tables (Result: {} rows) in {:?}",
        row_count, join_row_count, join_row_count, duration_join
    );

    // Expectation: Hash Join should be very fast (< 1s). Nested Loop would be 50k * 10k = 500M ops (slow).
    assert!(
        duration_join.as_secs() < 2,
        "Join is too slow! Hash Join might be broken."
    );
    ...
    for i in 0..updates {
        db.execute(&format!(
            "UPDATE users SET name = 'Updated {}' WHERE id = 0",
            i
        ))
        .await
        .unwrap();
    ...
    assert_eq!(
        freed, updates,
        "Vacuum should free exactly the number of overwritten versions"
    );
