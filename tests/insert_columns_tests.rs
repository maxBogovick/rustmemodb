...
async fn test_insert_with_column_list_values() {
    ...
    db.execute("CREATE TABLE t (id INTEGER, name TEXT, age INTEGER)").await.unwrap();
    db.execute("INSERT INTO t (name, id) VALUES ('Alice', 1)").await.unwrap();
...
async fn test_insert_with_column_list_select() {
    let mut db = InMemoryDB::new();
    db.execute("CREATE TABLE t (id INTEGER, name TEXT, age INTEGER)").await.unwrap();
    db.execute("INSERT INTO t (name, id) SELECT 'Bob', 2").await.unwrap();
...
async fn test_update_casts_to_column_type() {
    ...
    db.execute("UPDATE t SET v = 1.9 WHERE id = 1").await.unwrap();
