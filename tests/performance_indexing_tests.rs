...
async fn test_index_performance() {
    ...
    db.execute("CREATE TABLE large_table (id INTEGER, val INTEGER)")
        .await
        .unwrap();

    // 2. Insert data
    for i in 0..row_count {
        db.execute(&format!("INSERT INTO large_table VALUES ({}, {})", i, i))
            .await
            .unwrap();
    }

    // Warmup query
    db.execute("SELECT * FROM large_table LIMIT 1")
        .await
        .unwrap();
    ...
    let result_no_index = db
        .execute(&format!(
            "SELECT * FROM large_table WHERE val = {}",
            target_value
        ))
        .await
        .unwrap();
...
async fn perf_index_scan_after_updates() {
    ...
    for i in 0..row_count {
        let val = i % 1_000;
        db.execute(&format!("INSERT INTO large_table VALUES ({}, {})", i, val))
            .await
            .unwrap();
    }
    ...
    for i in (0..row_count).step_by(update_stride) {
        db.execute(&format!(
            "UPDATE large_table SET val = {} WHERE id = {}",
            target_value, i
        ))
        .await
        .unwrap();
    ...
    assert!(
        duration_with_index < duration_no_index,
        "Index scan should be faster after updates"
    );
}
