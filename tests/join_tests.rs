...
async fn setup_db() -> InMemoryDB {
    ...
    // Create 'users' table
    db.execute("CREATE TABLE users (id INTEGER, name TEXT, department_id INTEGER)")
        .await
        .unwrap();
    ...
    db.execute("INSERT INTO users VALUES (2, 'Bob', 1)")
        .await
        .unwrap();
    db.execute("INSERT INTO users VALUES (3, 'Charlie', 2)")
        .await
        .unwrap();
    db.execute("INSERT INTO users VALUES (4, 'David', NULL)")
        .await
...
async fn test_right_join() {
    ...
    // Find Marketing row
    let marketing_row = result
        .rows()
        .iter()
        .find(|r| r[1] == Value::Text("Marketing".into()));
...
async fn test_cross_join() {
    ...
    let result = db
        .execute("SELECT * FROM users CROSS JOIN departments")
        .await
        .unwrap();
...
async fn test_self_join() {
    ...
    db.execute("INSERT INTO employees VALUES (2, 'Manager', 1)")
        .await
        .unwrap();
    db.execute("INSERT INTO employees VALUES (3, 'Worker', 2)")
        .await
        .unwrap();
