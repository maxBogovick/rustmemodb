...
async fn test_timestamp_type() -> Result<()> {
    ...
    client.execute("CREATE TABLE logs (id INTEGER, event_time TIMESTAMP, message TEXT)").await?;
    ...
    client.execute(&format!("INSERT INTO logs VALUES (1, '{}', 'Log 1')", now_str)).await?;
...
async fn test_uuid_type() -> Result<()> {
    let client = Client::connect_local("admin", "adminpass").await?;
    client.execute("CREATE TABLE users (id UUID, name TEXT)").await?;

    let uuid = Uuid::new_v4();
    client.execute(&format!("INSERT INTO users VALUES ('{}', 'Alice')", uuid)).await?;
