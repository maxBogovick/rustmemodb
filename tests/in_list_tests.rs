...
async fn test_in_list_basic() {
    ...
    let res = db
        .execute("SELECT * FROM t WHERE v IN (10, 30)")
        .await
        .unwrap();
...
async fn test_not_in_list() {
    ...
    let res = db
        .execute("SELECT * FROM t WHERE v NOT IN (10, 30)")
        .await
        .unwrap();
...
async fn test_in_list_null_semantics() {
    ...
    let res_match = db
        .execute("SELECT * FROM t WHERE v IN (NULL, 10)")
        .await
        .unwrap();
    assert_eq!(res_match.row_count(), 1);

    let res_no_match = db
        .execute("SELECT * FROM t WHERE v IN (NULL, 30)")
        .await
        .unwrap();
