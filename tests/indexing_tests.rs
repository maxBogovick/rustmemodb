...
async fn test_create_index_and_use_it() {
    ...
    db.execute("INSERT INTO users VALUES (3, 'Charlie', 35)")
        .await
        .unwrap();
    db.execute("INSERT INTO users VALUES (4, 'Diana', 25)")
        .await
        .unwrap();
...
async fn test_index_visibility_after_update() {
    ...
    let rows = storage
        .scan_table_with_ids("users", &snapshot_tx1)
        .await
        .unwrap();
...
async fn test_index_performance_comparison() {
    ...
    let result_no_index = db
        .execute("SELECT * FROM large_table WHERE val = 42")
        .await
        .unwrap();
    ...
    db.execute("CREATE INDEX idx_val ON large_table (val)")
        .await
        .unwrap();
    ...
    if duration_with_index < duration_no_index {
        println!(
            "SUCCESS: Indexing improved performance by {:.2}x",
