...
async fn test_group_by_simple() {
    ...
    db.execute("INSERT INTO sales VALUES ('North', 200)")
        .await
        .unwrap();
    db.execute("INSERT INTO sales VALUES ('South', 50)")
        .await
        .unwrap();
    db.execute("INSERT INTO sales VALUES ('South', 150)")
        .await
        .unwrap();
    db.execute("INSERT INTO sales VALUES ('East', 300)")
        .await
        .unwrap();

    // SELECT region, SUM(amount) FROM sales GROUP BY region ORDER BY region
    let result = db
        .execute("SELECT region, SUM(amount) FROM sales GROUP BY region ORDER BY region")
        .await
        .unwrap();
...
async fn test_group_by_count() {
    let mut db = InMemoryDB::new();
    db.execute("CREATE TABLE logs (level TEXT, msg TEXT)")
        .await
        .unwrap();
    db.execute("INSERT INTO logs VALUES ('INFO', 'a')")
        .await
        .unwrap();
    db.execute("INSERT INTO logs VALUES ('INFO', 'b')")
        .await
        .unwrap();
    ...
    let result = db
        .execute("SELECT level, COUNT(*) FROM logs GROUP BY level ORDER BY level")
        .await
        .unwrap();
...
async fn test_group_by_having() {
    ...
    db.execute("INSERT INTO orders VALUES (1, 200)")
        .await
        .unwrap();
    db.execute("INSERT INTO orders VALUES (2, 50)")
        .await
        .unwrap();
    db.execute("INSERT INTO orders VALUES (3, 500)")
        .await
        .unwrap();
