...
async fn test_scalar_subquery() -> Result<()> {
    ...
    let result = client.query("SELECT * FROM users WHERE id = (SELECT id FROM users WHERE name = 'Alice')").await?;
...
async fn test_in_subquery() -> Result<()> {
    ...
    client.execute("CREATE TABLE users (id INTEGER, name TEXT)").await?;
    client.execute("CREATE TABLE orders (id INTEGER, user_id INTEGER)").await?;
    ...
    // IN subquery
    let result = client.query("SELECT * FROM users WHERE id IN (SELECT user_id FROM orders)").await?;
...
async fn test_derived_table() -> Result<()> {
    ...
    let result = client.query("SELECT * FROM (SELECT * FROM users WHERE id > 1) AS t").await?;
    assert_eq!(result.row_count(), 1);
    let row = &result.rows()[0];
...
async fn test_exists_subquery() -> Result<()> {
    ...
    // EXISTS
    let result = client.query("SELECT * FROM users WHERE EXISTS (SELECT 1 FROM users WHERE id = 1)").await?;
    assert_eq!(result.row_count(), 1);

    let result = client.query("SELECT * FROM users WHERE EXISTS (SELECT 1 FROM users WHERE id = 999)").await?;
