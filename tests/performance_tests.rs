...
#[tokio::test]
async fn test_performance_bulk_insert() {
    let client = Client::connect("admin", "adminpass").await.unwrap();

    client
        .execute("CREATE TABLE perf_insert (id INTEGER, data TEXT)")
        .await
        .unwrap();
...
#[tokio::test]
async fn test_performance_bulk_select() {
    let client = Client::connect("admin", "adminpass").await.unwrap();

    client
        .execute("CREATE TABLE perf_select (id INTEGER, data TEXT)")
        .await
        .unwrap();

    // Insert test data
    for i in 0..1000 {
        // Reduced from 10000 for faster tests
        client
            .execute(&format!(
...
#[tokio::test]
async fn test_performance_filtered_select() {
    let client = Client::connect("admin", "adminpass").await.unwrap();

    client
        .execute("CREATE TABLE perf_filter (id INTEGER, category TEXT, value INTEGER)")
        .await
        .unwrap();
    ...
    for _ in 0..count {
        let _result = client
            .query("SELECT * FROM perf_filter WHERE category = 'A' AND value > 1000")
            .await
            .unwrap();
    }
...
#[tokio::test]
async fn test_performance_order_by() {
    let client = Client::connect("admin", "adminpass").await.unwrap();

    client
        .execute("CREATE TABLE perf_sort (id INTEGER, value INTEGER)")
        .await
        .unwrap();
    ...
    for i in 0..500 {
        ...
        client
            .execute(&format!("INSERT INTO perf_sort VALUES ({}, {})", i, value))
            .await
            .unwrap();
    }
    ...
    for _ in 0..count {
        let _result = client
            .query("SELECT * FROM perf_sort ORDER BY value ASC")
            .await
            .unwrap();
    }
...
async fn test_performance_connection_pool() {
    ...
    client
        .execute("CREATE TABLE perf_pool (id INTEGER)")
        .await
        .unwrap();
    ...
    for i in 0..count {
        ...
        conn.execute(&format!("INSERT INTO perf_pool VALUES ({})", i))
            .await
            .unwrap();
    }
    ...
    println!(
        "Executed {} operations through pool in {:?}",
        count, duration
    );
}

#[tokio::test]
async fn test_performance_table_scan_sizes() {
    let client = Client::connect("admin", "adminpass").await.unwrap();
    ...
    for size in sizes {
        client
            .execute(&format!(
                "CREATE TABLE perf_scan_{} (id INTEGER, data TEXT)",
                size
            ))
            .await
            .unwrap();
        ...
        for _ in 0..iterations {
            let _result = client
                .query(&format!("SELECT * FROM perf_scan_{}", size))
                .await
                .unwrap();
        }
...
#[tokio::test]
async fn test_performance_transaction_overhead() {
    let client = Client::connect("admin", "adminpass").await.unwrap();

    client
        .execute("CREATE TABLE perf_tx_overhead (id INTEGER)")
        .await
        .unwrap();
    ...
    for i in 0..100 {
        client
            .execute(&format!("INSERT INTO perf_tx_overhead VALUES ({})", i))
            .await
            .unwrap();
    }
    ...
    // With transaction
    client
        .execute("CREATE TABLE perf_tx_with (id INTEGER)")
        .await
        .unwrap();
    let mut conn = client.get_connection().await.unwrap();
    ...
    for i in 0..100 {
        conn.execute(&format!("INSERT INTO perf_tx_with VALUES ({})", i))
            .await
            .unwrap();
    }
