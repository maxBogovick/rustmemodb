...
use rustmemodb::core::{DbError, Result as DbResult, Value};
use rustmemodb::server::PostgresServer;
use rustmemodb::{Client, ConnectionConfig, ConnectionPool, DurabilityMode, InMemoryDB};
use std::sync::Arc;
...
async fn fail_02_drop_connection_should_rollback_and_release_unique() {
    ...
    assert!(
        result.is_ok(),
        "expected rollback on drop to release unique key"
    );
}
...
async fn fail_03_lost_update_should_be_detected() {
    ...
    c1.execute("UPDATE t SET v = 110 WHERE id = 1")
        .await
        .unwrap();
    c2.execute("UPDATE t SET v = 120 WHERE id = 1")
        .await
        .unwrap();
    ...
    assert!(
        res.is_err(),
        "expected write-write conflict on second commit"
    );
}
...
async fn fail_04_null_boolean_semantics() {
    ...
    assert_eq!(
        result.row_count(),
        1,
        "NULL OR TRUE should evaluate to TRUE"
    );
}
...
async fn fail_05_unique_with_aborted_tx_should_succeed() {
    let client = Client::connect_local("admin", "adminpass").await.unwrap();
    client
        .execute("CREATE TABLE t (id INTEGER UNIQUE)")
        .await
        .unwrap();
    ...
    assert!(
        res2.is_ok(),
        "second insert should wait or succeed if first tx aborts"
    );
...
async fn fail_07_fk_update_should_be_enforced() {
    ...
    client
        ...
        .unwrap();
    client
        .execute("CREATE TABLE child (id INTEGER, parent_id INTEGER REFERENCES parent(id))")
        .await
        .unwrap();
    client
    ...
    let res = client
        .execute("UPDATE child SET parent_id = 999 WHERE id = 10")
        .await;
...
async fn fail_08_alter_add_column_not_null_should_be_enforced() {
    ...
    assert!(
        res.is_err(),
        "NOT NULL should reject implicit NULL on added column"
    );
}
...
async fn fail_09_create_table_default_should_apply() {
    let mut db = InMemoryDB::new();
    db.execute("CREATE TABLE t (id INTEGER, v INTEGER DEFAULT 7)")
        .await
        .unwrap();
...
async fn fail_10_prepared_statement_timestamp_param() {
    let client = Client::connect_local("admin", "adminpass").await.unwrap();
    client
        .execute("CREATE TABLE t (id INTEGER, ts TIMESTAMP)")
        .await
        .unwrap();
    ...
    assert!(
        matches!(res.rows()[0][0], Value::Timestamp(_)),
...
async fn fail_12_explain_should_use_index_with_parameter() {
    ...
    assert!(
        plan.contains("index_scan: Some"),
        "expected index scan for parameterized predicate"
    );
}
...
async fn fail_13_write_lock_should_not_block_unrelated_read() {
    ...
    let db = Arc::new(RwLock::new(InMemoryDB::new()));
    {
        ...
        db_guard
            ...
            .unwrap();
        db_guard
            .execute("CREATE TABLE b (id INTEGER)")
            .await
            .unwrap();
    }
    ...
    let pool = Arc::new(
        ConnectionPool::new_with_db(config, db.clone())
            .await
            .unwrap(),
    );
...
async fn fail_15_auto_vacuum_should_clean_versions() {
    ...
    assert_eq!(
        freed, 0,
        "expected auto-vacuum to have cleaned old versions"
    );
...
async fn fail_16_default_admin_should_be_disabled() {
    ...
    assert!(
        res.is_err(),
        "default admin credentials should not be accepted"
    );
...
async fn fail_17_ssl_should_be_supported() {
    let Some(port) = reserve_port().await else {
        return;
    };
...
async fn fail_18_users_should_persist_across_restart() {
    ...
    let auth = AuthManager::new();
    auth.create_user("persist_user", "persistpass", vec![])
        .await
        .unwrap();
...
async fn fail_20_conflict_should_survive_wal_recovery() {
    ...
    let config = ConnectionConfig::new("admin", "adminpass");
    let pool = ConnectionPool::new_with_db(config, db.clone())
        .await
        .unwrap();
    ...
    let pool_after = ConnectionPool::new_with_db(config_after, db_after)
        .await
        .unwrap();
    ...
    assert_eq!(
        res.rows()[0][0],
        Value::Integer(2),
        "recovery should reject conflicting commit"
    );
}
...
async fn fail_21_metrics_should_be_queryable() {
    ...
    assert!(
        res.is_ok(),
        "expected a system metrics table or view for monitoring"
    );
}
