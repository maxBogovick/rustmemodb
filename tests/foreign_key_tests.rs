...
async fn test_foreign_key_constraint() -> Result<()> {
    ...
    // 3. Valid Insert
    client
        .execute("INSERT INTO posts VALUES (100, 1, 'Alice Post')")
        .await?;
    client
        .execute("INSERT INTO posts VALUES (101, 2, 'Bob Post')")
        .await?;

    // 4. Invalid Insert (Non-existent user)
    let res = client
        .execute("INSERT INTO posts VALUES (102, 999, 'Ghost Post')")
        .await;
    match res {
    ...
    // First delete child
    client
        .execute("DELETE FROM posts WHERE user_id = 1")
        .await?;
...
async fn test_foreign_key_null_handling() -> Result<()> {
    ...
    client
        ...
        .await?;
    client
        .execute("CREATE TABLE items (id INTEGER, category_id INTEGER REFERENCES categories(id))")
        .await?;
