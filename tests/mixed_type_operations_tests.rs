...
fn test_boolean_comparison_false() {
    ...
    let result = client.query("SELECT * FROM test_bool WHERE active = false").unwrap();
    println!("Rows returned: {}", result.row_count());
    assert_eq!(result.row_count(), 1);
}
...
fn test_float_comparison() {
    ...
    client.execute("INSERT INTO test_float VALUES (2, 87.3)").unwrap();
    client.execute("INSERT INTO test_float VALUES (3, 92.1)").unwrap();

    println!("Testing: WHERE score > 90.0");
    let result = client.query("SELECT * FROM test_float WHERE score > 90.0").unwrap();
    println!("Rows returned: {}", result.row_count());
    assert_eq!(result.row_count(), 2);
}
...
fn test_combined_boolean_and_float() {
    ...
    client.execute("INSERT INTO mixed_data VALUES (1, 'Alice', 95.5, true)").unwrap();
    client.execute("INSERT INTO mixed_data VALUES (2, 'Bob', 87.3, false)").unwrap();
    client.execute("INSERT INTO mixed_data VALUES (3, 'Charlie', 92.1, true)").unwrap();

    // Test each condition separately
    println!("\n=== Testing: WHERE active = true ===");
    let result = client.query("SELECT name, score FROM mixed_data WHERE active = true").unwrap();
    println!("Rows: {}", result.row_count());
    println!("Expected: 2 (Alice, Charlie)");

    println!("\n=== Testing: WHERE score > 90 ===");
    let result = client.query("SELECT name, score FROM mixed_data WHERE score > 90").unwrap();
    println!("Rows: {}", result.row_count());
    println!("Expected: 2 (Alice, Charlie)");
...
fn test_mixed_type_arithmetic() {
    ...
    client.execute("INSERT INTO test_arithmetic VALUES (2, 20.3)").unwrap();

    // Test Float + Integer
    println!("\n=== Testing: WHERE value + 5 > 15 ===");
    let result = client.query("SELECT * FROM test_arithmetic WHERE value + 5 > 15").unwrap();
    println!("Rows: {}", result.row_count());
    ...
    let result = client.query("SELECT * FROM test_arithmetic WHERE value * 2 > 30").unwrap();
    println!("Rows: {}", result.row_count());
