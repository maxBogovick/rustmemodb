...
async fn test_cow_forking_isolation() -> Result<()> {
    ...
    // Seed data
    master.execute("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)").await?;
    master.execute("INSERT INTO users VALUES (1, 'Alice')").await?;
    master.execute("INSERT INTO users VALUES (2, 'Bob')").await?;
    ...
    test_a.execute("INSERT INTO users VALUES (3, 'Charlie')").await?;
    ...
    assert_eq!(result.rows()[0].get(0).unwrap().as_str().unwrap(), "Charlie");
    ...
    // Modify B
    test_b.execute("UPDATE users SET name = 'Bobby' WHERE id = 2").await?;
    ...
    assert_eq!(master_check.rows()[0].get(1).unwrap().as_str().unwrap(), "Alice"); // Not deleted
    assert_eq!(master_check.rows()[1].get(1).unwrap().as_str().unwrap(), "Bob");   // Not updated
...
async fn test_fork_bench_simulation() -> Result<()> {
    let master = Client::connect_local("admin", "adminpass").await?;
    master.execute("CREATE TABLE huge_table (id INTEGER)").await?;
    ...
    for i in 0..50_000 {
        ...
        master.execute(&format!("INSERT INTO huge_table VALUES ({})", i)).await?;
    }
    ...
    assert!(duration.as_millis() < 2000, "Forking took too long! COW might not be working.");
