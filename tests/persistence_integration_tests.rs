...
async fn test_persistence_create_table() {
    ...
    // Enable persistence
    db.enable_persistence(temp_dir.path(), DurabilityMode::Sync).await
        .unwrap();
...
async fn test_checkpoint_creates_snapshot() {
    ...
    db.enable_persistence(temp_dir.path(), DurabilityMode::Async).await
        .unwrap();

    // Create some tables
    db.execute("CREATE TABLE users (id INTEGER, name TEXT)").await
        .unwrap();
    db.execute("CREATE TABLE products (id INTEGER, price FLOAT)").await
        .unwrap();
...
async fn test_recovery_from_snapshot_and_wal() {
    ...
    // Session 1: Create checkpoint
    {
        ...
        db.execute("CREATE TABLE orders (id INTEGER, total FLOAT)").await
            .unwrap();
...
async fn test_durability_mode_none() {
    ...
    db.enable_persistence(temp_dir.path(), DurabilityMode::None).await
        .unwrap();
...
async fn test_checkpoint_after_many_operations() {
    ...
    for i in 0..10 {
        db.execute(&format!("CREATE TABLE table_{} (id INTEGER)", i)).await
            .unwrap();
    }
...
async fn test_cannot_enable_persistence_twice() {
    ...
    let result = db.enable_persistence(temp_dir.path(), DurabilityMode::Sync).await;
...
async fn test_update_persistence_and_recovery() {
    ...
    // Session 1: Insert and update data
    {
        ...
        // Update prices
        db.execute("UPDATE products SET price = 899.99 WHERE id = 1").await.unwrap();
        db.execute("UPDATE products SET price = 19.99 WHERE id = 2").await.unwrap();

        // Verify updates
        let result = db.execute("SELECT * FROM products WHERE id = 1").await.unwrap();
    ...
    // Session 2: Recover and verify updates
    {
        ...
        let result = db.execute("SELECT * FROM products WHERE id = 2").await.unwrap();
        let row = &result.rows()[0];
...
async fn test_delete_persistence_and_recovery() {
    ...
    // Session 1: Insert and delete data
    {
        ...
        db.execute("CREATE TABLE items (id INTEGER, name TEXT)").await
            .unwrap();
        db.execute("INSERT INTO items VALUES (1, 'Item1')").await.unwrap();
        db.execute("INSERT INTO items VALUES (2, 'Item2')").await.unwrap();
        db.execute("INSERT INTO items VALUES (3, 'Item3')").await.unwrap();
        db.execute("INSERT INTO items VALUES (4, 'Item4')").await.unwrap();
...
async fn test_mixed_dml_operations_recovery() {
    ...
    // Session 1: Mixed INSERT, UPDATE, DELETE operations
    {
        ...
        db.execute("CREATE TABLE accounts (id INTEGER, name TEXT, balance FLOAT)").await
            .unwrap();

        // Insert initial data
        db.execute("INSERT INTO accounts VALUES (1, 'Alice', 1000.0)").await.unwrap();
        db.execute("INSERT INTO accounts VALUES (2, 'Bob', 500.0)").await.unwrap();
        db.execute("INSERT INTO accounts VALUES (3, 'Charlie', 750.0)").await.unwrap();

        // Update balances
        db.execute("UPDATE accounts SET balance = 1200.0 WHERE id = 1").await.unwrap();
        db.execute("UPDATE accounts SET balance = 600.0 WHERE id = 2").await.unwrap();

        // Delete one account
        db.execute("DELETE FROM accounts WHERE id = 3").await.unwrap();

        // Insert new account
        db.execute("INSERT INTO accounts VALUES (4, 'David', 300.0)").await.unwrap();
...
async fn test_dml_with_checkpoint() {
    ...
    // Session 1: Operations with checkpoint
    {
        ...
        db.execute("CREATE TABLE logs (id INTEGER, message TEXT)").await
            .unwrap();
        db.execute("INSERT INTO logs VALUES (1, 'First')").await.unwrap();
        db.execute("INSERT INTO logs VALUES (2, 'Second')").await.unwrap();

        // Create checkpoint
        db.checkpoint().await.unwrap();

        // More operations after checkpoint
        db.execute("INSERT INTO logs VALUES (3, 'Third')").await.unwrap();
        db.execute("UPDATE logs SET message = 'FIRST' WHERE id = 1").await.unwrap();
