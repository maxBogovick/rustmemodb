...
async fn email_uniqueness_is_enforced_across_store_reopen() {
    ...
    let service_a = UserService::open(data_dir.clone())
        .await
        .expect("first service should initialize");
    ...
    let service_b = UserService::open(data_dir)
        .await
        .expect("second service should initialize");
