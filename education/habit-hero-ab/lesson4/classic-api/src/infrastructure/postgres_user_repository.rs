...
impl UserRepository for PostgresUserRepository {
    ...
    async fn apply_lifecycle_command(
        ...
        let maybe_row = sqlx::query(
            r#"
            UPDATE users
            SET active = $1, version = version + 1
            WHERE id = $2 AND version = $3
            RETURNING id, email, display_name, active, version, created_at, updated_at
            "#,
        )
        .bind(command.target_active())
        .bind(id)
        .bind(expected_version)
        ...
        let updated = row_to_user(&row);

        sqlx::query(
            r#"
            INSERT INTO user_events (id, user_id, event_type, message, resulting_version)
            VALUES ($1, $2, $3, $4, $5)
            "#,
        )
        .bind(Uuid::new_v4())
    ...
    async fn bulk_apply_lifecycle_command(
        ...
        if !updated_rows.is_empty() {
            ...
            insert_builder.push_values(updated_rows.iter(), |mut builder, row| {
                builder.push_bind(Uuid::new_v4());
                builder.push_bind(row.get::<Uuid, _>("id"));
                builder.push_bind(command.bulk_event_type());
                builder.push_bind(command.bulk_event_message());
                builder.push_bind(row.get::<i64, _>("version"));
            });
        ...
        Ok(u64::try_from(updated_rows.len()).unwrap_or(u64::MAX))
    }

    async fn list_events(&self, id: Uuid, limit: u32) -> Result<Vec<UserAuditEvent>, DomainError> {
        let rows = sqlx::query(
            r#"
            SELECT id, user_id, event_type, message, resulting_version, created_at
            FROM user_events
            WHERE user_id = $1
...
fn row_to_event(row: &sqlx::postgres::PgRow) -> UserAuditEvent {
    UserAuditEvent {
        id: row.get::<Uuid, _>("id").to_string(),
        user_id: row.get::<Uuid, _>("user_id"),
        event_type: row.get::<String, _>("event_type"),
        message: row.get::<String, _>("message"),
        resulting_version: row.get::<i64, _>("resulting_version"),
        created_at: row.get::<DateTime<Utc>, _>("created_at"),
    }
}
