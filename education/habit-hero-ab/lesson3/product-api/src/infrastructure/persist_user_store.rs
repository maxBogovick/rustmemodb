...
impl UserRepository for PersistUserStore {
    ...
    async fn update(
        ...
        let Some(existing) = state.users.get(&id).cloned() else {
            return Ok(None);
        };
    ...
    async fn delete(&self, id: Uuid, expected_version: i64) -> Result<bool, DomainError> {
        ...
        let deleted = state.users.delete(&id).await.map_err(map_db_error)?;
        if deleted {
            // Best-effort cleanup of email claim on delete.
            // If this fails, user row is already removed, so we avoid surfacing a false 500.
            // The claim can be reconciled by maintenance tooling in a real deployment.
            let _ = state.email_claims.delete(existing.email()).await;
        }
