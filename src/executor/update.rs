...
use crate::parser::ast::{Statement, UpdateStmt};
...
impl UpdateExecutor {
    async fn execute_update(
        ...
        let rows = ctx
            .storage
            .scan_table_with_ids(&update.table_name, &ctx.snapshot)
            .await?;
            ...
            if should_update {
                ...
                for assignment in &update.assignments {
                    let col_idx = schema
                        ...
                        .ok_or_else(|| {
                            crate::core::DbError::ExecutionError(format!(
                                "Column '{}' not found",
                                assignment.column
                            ))
                        })?;

                    let new_value = eval_ctx
                        .evaluate(&assignment.value, &row, schema.schema())
                        .await?;
                    ...
                    if let Some(ref fk) = column.references {
                        ...
                        let exists = if let Some(rows) = ctx
                            ...
                            .scan_index(
                                ...
                                val,
                                &None,
                                &crate::planner::logical_plan::IndexOp::Eq,
                                &ctx.snapshot,
                            )
                            .await?
                        {
                            ...
                            let col_idx = ref_schema
                                ...
                                .ok_or_else(|| {
                                    crate::core::DbError::ColumnNotFound(
                                        fk.column.clone(),
                                        fk.table.clone(),
                                    )
                                })?;
        ...
        for (idx, old_row, new_row) in &updates {
            ...
            let success = ctx
                .storage
                .update_row(&update.table_name, *idx, new_row.clone(), &ctx.snapshot)
                .await?;
            if !success {
