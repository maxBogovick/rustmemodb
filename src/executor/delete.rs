...
impl DeleteExecutor {
    async fn execute_delete(
        ...
        let rows = ctx
            .storage
            .scan_table_with_ids(&delete.table_name, &ctx.snapshot)
            .await?;
                    ...
                    if let Some(ref fk) = column.references {
                        if fk.table == delete.table_name {
                                ...
                                .await?
                            {
                                ...
                                // Fallback scan
                                let child_rows =
                                    ctx.storage.scan_table(table_name, &ctx.snapshot).await?;
        ...
        for idx in &deleted_indices {
            let _success = ctx
                .storage
                .delete_row(&delete.table_name, *idx, ctx.snapshot.tx_id)
                .await?;
        }
