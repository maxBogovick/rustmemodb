...
```bash
cargo run --offline --manifest-path examples/todo_persist_runtime/Cargo.toml
```
...
```bash
cargo run --offline \
  --manifest-path examples/todo_persist_runtime/Cargo.toml \
...
- `TODO_DATA_DIR` (default: `examples/todo_persist_runtime/.data/primary`)
...
- `TODO_REPLICA_DIRS` (comma-separated, например `examples/todo_persist_runtime/.data/replica-a`)
...
```bash
bash examples/todo_persist_runtime/scripts/failover_demo.sh
```
...
- `DATA_ROOT` (default `examples/todo_persist_runtime/.data/failover_demo`)
